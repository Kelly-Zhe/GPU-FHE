...
def polys_add_mod(a, b, MOD):
    ...
    c = np.zeros((a.shape[0], a.shape[1]), dtype=np.uint64)
    for i, val in enumerate(MOD):
        c[i] = vec_add_mod(a[i], b[i], val)
    return c
...
def polys_sub_mod(a, b, MOD):
    ...
    c = np.zeros((a.shape[0], a.shape[1]), dtype=np.uint64)
    for i, val in enumerate(MOD):
        c[i] = vec_sub_mod(a[i], b[i], val)
    return c


def polys_mul_mod(a, b, MOD):
    assert a.shape == b.shape and a.shape[0] == len(MOD)
    c = np.zeros((a.shape[0], a.shape[1]), dtype=np.uint64)
    for i, val in enumerate(MOD):
        c[i] = vec_mul_mod(a[i], b[i], val)
    return c
...
def homo_mult_core(in0, in1, moduliQ):
    assert in0.curr_limbs == in1.curr_limbs
    curr_limbs = in0.curr_limbs
    MOD = moduliQ[:curr_limbs]
    ...
    res[0] = polys_mul_mod(res[0], axbx2, MOD)
    res[2] = polys_mul_mod(in0.cv[0], in1.cv[0], MOD)  # axax, use in the next KS step
    res[1] = polys_mul_mod(in0.cv[1], in1.cv[1], MOD)  # res.bx
...
def homo_mult(in0, in1,
    ...
    res = homo_mult_core(in0, in1, moduliQ)
...
def rescale(a,
    ...
    for i in range(curr_limbs - 1):
        ...
        res[i] = vec_mul_scalar_mod(res[i], qInvModq[curr_limbs - 1][i], moduliQ[i])
...
def DropLastElementAndScale(
    ...
    for i in range(curr_limbs - 1):
        tmp = vec_mul_scalar_mod(a[i], qInvModq[curr_limbs - 1][i], moduliQ[i])
